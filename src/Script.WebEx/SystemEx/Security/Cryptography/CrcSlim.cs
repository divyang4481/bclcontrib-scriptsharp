...
#endif
{
    public class CrcSlim
    {
        private static ushort CRC_INIT_VALUE = (ushort)0xffff;
        //private static ushort CRC_XOR_VALUE = (ushort)0x0000;
        private static uint[] crctable = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
        ...
        public static int CRC_Block(byte[] start, int count)
        {
            ushort crc = CRC_INIT_VALUE;
            int ndx = 0;
            while (count-- > 0)
                crc = (ushort)((crc << 8) ^ crctable[0xff & ((crc >> 8) ^ start[ndx++])]);
            // unsigned short
            return crc & (ushort)0xFFFF;
        }
        ...
        private static byte[] chkb = new byte[60 + 4];
