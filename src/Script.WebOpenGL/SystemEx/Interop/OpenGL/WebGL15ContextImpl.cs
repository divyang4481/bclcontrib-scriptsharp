...
#endif
{
    public partial class WebGL15ContextImpl : WebGL15Context
    {
        #region Logging
        ...
        {
            return (_texEnvMode[indx] == (int)GLES20.REPLACE ? 0 : (_textureFormats[_boundTextureId[indx]] == 3 ? 2 : 1));
        }
        ...
        private void VertexAttribStaticDrawPointer(uint indx, int size, uint type, bool normalized, int stride, long offset, Stream pointer, int staticDrawIndex)
        {
            ...
            if (b == null)
            {
                ...
                gl.BindBuffer(GLES20.ARRAY_BUFFER, b);
                gl.BufferData(GLES20.ARRAY_BUFFER, GetTypedArray(pointer, type), GLES20.STATIC_DRAW); CheckError("bufferData");
        ...
        // GL_UNKNOWN
        public override void ClientActiveTexture(uint texture) { _clientActiveTexture = texture - GLES20.TEXTURE0; }
        ...
        public override void TexParameterfv(uint target, uint pname, Stream s) { }
        ...
        public override void TexParameteriv(uint target, uint pname, Stream s) { }
        ...
        {
            _textureFormat[_boundTextureId[_activeTexture]] = internalformat;
            gl.TexImage2D(target, level, internalformat, width, height, border, format, type, GetTypedArray(pixels, type)); CheckError("glTexImage2D");
        }
#if CODE_ANALYSIS
        ...
        public override void TexImage2Dex(uint target, int level, uint internalformat, uint format, uint type, CanvasElementEx canvas) { gl.TexImage2D(target, level, internalformat, format, type, canvas); CheckError("texImage2D"); }
#endif
        // framebuf commands
        public override void DrawBuffer(uint mode) { /*TODO*/ }
        ...
        // xform commands
        public override void DepthRange(float zNear, float zFar) { gl.DepthRange(zNear, zFar); CheckError("glDepthRangef"); }
        public override void Viewport(int x, int y, int width, int height)
        {
            base.Viewport(x, y, width, height);
            gl.Viewport(x, y, width, height); CheckError("glViewport");
        }
        ...
        public override void DrawArrays(uint mode, int first, int count)
        {
            ...
            PrepareDraw();
            gl.DrawArrays(mode, first, count); CheckError("drawArrays");
        }
        public override void DrawElements(uint mode, int count, uint type, Stream indicies)
        {
            PrepareDraw();
            gl.BindBuffer(GL15.ELEMENT_ARRAY_BUFFER, _elementBuffer); CheckError("bindBuffer(el)");
            gl.BufferData(GL15.ELEMENT_ARRAY_BUFFER, GetTypedArray(indicies, GL15.UNSIGNED_SHORT), GL15.DYNAMIC_DRAW); CheckError("bufferData(el)");
            //count = indicies.Remaining();
            gl.DrawElements(mode, count, GL15.UNSIGNED_SHORT, 0); CheckError("drawElements");
        }
        ...
        {
            gl.ActiveTexture(texture); CheckError("glActiveTexture");
            _activeTexture = texture - GL15.TEXTURE0;
        }


        // GL_VERSION_1_4
        public override void PointParameterf(uint pname, float param) { }
        public override void PointParameterfv(uint pname, Stream s) { }
    }
}
